(() => {
  ...
  class InputManager {
    constructor(scene) {
      ...
      this.pad = null; this.pauseLatch = false;
    ...
    jumpPressed() { return Phaser.Input.Keyboard.JustDown(this.keys.jump) || !!this.pad?.A; }
    shootHeld() { return this.keys.shoot.isDown || !!this.pad?.X; }
    bombPressed() { return Phaser.Input.Keyboard.JustDown(this.keys.bomb) || Phaser.Input.Gamepad.Button.JustDown(this.pad?.R1); }
    pausePressed() { const pressed = Phaser.Input.Keyboard.JustDown(this.keys.pauseEsc) || Phaser.Input.Keyboard.JustDown(this.keys.pauseEnter) || Phaser.Input.Gamepad.Button.JustDown(this.pad?.START); if (pressed && !this.pauseLatch) { this.pauseLatch = true; this.scene.time.delayedCall(180, () => { this.pauseLatch = false; }); return true; } return false; }
  ...
  class Projectile extends Phaser.Physics.Arcade.Image {
    ...
    preUpdate(time, delta) { super.preUpdate(time, delta); this.life -= delta; if (this.life <= 0 || this.x < -100 || this.x > this.scene.levelWidth + 100 || this.y < -50 || this.y > 650) this.destroy(); }
  }
